...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Abrir modal de cores quando clicar no select
  corSelect.addEventListener('click', function(e) {
    e.preventDefault();
    abrirModalCores();
  });
...
function abrirModalCores() {
  ...
  document.body.classList.add('modal-cores-aberto');
...
function fecharModalCores() {
  ...
  document.body.classList.remove('modal-cores-aberto');
...
function confirmarCorSelecionada() {
  if (corSelecionadaModal) {
    ...
    for (let i = 0; i < corSelect.options.length; i++) {
      if (corSelect.options[i].value === corSelecionadaModal) {
        corSelect.selectedIndex = i;
        // Atualizar pré-visualização
        corPreviewImg.src = corSelect.options[i].dataset.imagem;
        corPreviewImg.alt = `Pré-visualização ${corSelecionadaModal}`;
        break;
      }
    }
  }
